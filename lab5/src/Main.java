...
public class Main {
    public static void main(String[] args) {
       Expressions.Variable var = new Expressions.Variable("x");
        System.out.println(new Power(new Sin(new Prod(2,var)),2).diff(var).simplify());
//        Main.diffPoly();
//        Main.diffCircle();
    }
    ...
    static void buildAndEvaluate() {
        ...
        for (double v = -5; v < 5; v += 0.1) {
            x.setValue(v);
            if (Math.abs(exp.evaluate()) < 1e-10) {
